...
fun PlayerBottomSheetScaffold(
    ...
    val sheetPeekHeight = when (bottomSheetState.currentValue) {
    ...
    val scaffoldBottomPadding =
        innerPadding.calculateBottomPadding() + if (bottomSheetState.currentValue == SheetValue.Hidden) {
            0.dp
        } else {
            when {
                // 완전히 펼쳐진 상태 (Expanded)
    ...
    LaunchedEffect(bottomSheetState) {
        snapshotFlow {
            transformOffset(
                bottomSheetState.requireOffset() / screenHeightPx,
                searchWidgetState = searchWidgetState,
                isKeyboardOpen
            )
        }
